...
> 検証方針: 過去の評価文書に依存せず、リポジトリのソースと個別コマンド（`mix test` / `cargo test` / `cargo clippy -D warnings` / `mix credo --strict` / `mix compile --warnings-as-errors` 等）で現状を確認した。ローカル CI 一括は **`mix alchemy.ci`**（[.workspace/0_docs/warranty/ci.md](../warranty/ci.md)）。  
> 2026-03 系の日付付き評価レポートは `.workspace/0_docs/evaluation/archive/` に移動済み。
...
| `mix alchemy.ci` | 単一エントリのローカル CI（`apps/core/lib/mix/tasks/alchemy.ci.ex`）。`rust` / `elixir` / `check` で部分実行可。手順は [.workspace/0_docs/warranty/ci.md](../warranty/ci.md)、[development.md](../../development.md) 参照 |
...
- **`mix alchemy.ci` による単一エントリ** `+1`  
  ...
  > 対象: `apps/core/lib/mix/tasks/alchemy.ci.ex`, `.workspace/0_docs/warranty/ci.md`
...
- [improvement-plan.md](../../../0_reference/improvement-plan.md)
