...
前回評価: Fable 5 / 2026-07-31（`.workspace/0_docs/evaluation/fable/archive/2026-07-31/`）
...
| 文書 | 出力先 |
|:---|:---|
| 第1評価者の 4 文書 | `.workspace/0_docs/evaluation/opus/` |
| 第2評価者の 4 文書 | `.workspace/0_docs/evaluation/gpt/` |
| 統合一覧（本まとめ） | `.workspace/0_docs/evaluation/specific-{strengths,weaknesses,proposals}-2026-08-25.md` |
...
| 保証ドキュメントと評価ルールが旧構成のまま | **-3** | `.workspace/0_docs/warranty/ci.md`, `.cursor/rules/evaluation.mdc` |
...
| 7 | `.workspace/0_docs/warranty/ci.md` / `evaluation.mdc` / README を実態に合わせる | -3 が消える。今回の評価で実際に齟齬を生じた | 1 時間 |
...
- **`.credo.exs` と `.workspace/0_docs/warranty/ci.md` の設定値を突き合わせ** — 4 点の不一致を確認（現 HEAD でも未修正）。
