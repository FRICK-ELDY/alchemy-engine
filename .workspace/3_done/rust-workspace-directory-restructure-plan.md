...
11. **`.workspace/0_docs/architecture/overview.md`** — ディレクトリツリー、mermaid のパス、本文の `native/` 表記。
12. **`.workspace/0_docs/architecture/` 以下**（`rust/nif.md`, `elixir/core.md`, `vision.md`, `draw-command-spec.md`, `erlang-term-schema.md` 等）— **`native/` → `rust/`** への一括置換は **レビュー付き**で（歴史的文脈で `native` を残したい箇所がないか確認）。
...
| ドキュメント大量の置換で誤変換 | 一括置換より、**grep リストを取ってから**段階的に。評価アーカイブ（`.workspace/0_docs/evaluation/*`）は優先度を下げてよい。 |
...
- アーキ概要: [.workspace/0_docs/architecture/overview.md](../0_docs/architecture/overview.md)
